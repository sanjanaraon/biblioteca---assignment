...
public class SubMenuTest {
    ...
    public void shouldCallCheckOutBookMenuActionWhen2IsPassedWithValidCredentials() throws Exception {
        ...
        when(bookLibraryManager.getItemListByCategory("book")).thenReturn(listOfBooks);
        when(bookLibraryManager.displayMainMenu()).thenReturn("Main menu ");
        when(bookLibraryManager.displayItemDetailsInTableForm(listOfBooks)).thenReturn("Book Details ");
        ...
        String expected = "Main menu Enter library numberEnter passwordBook Details Select a Item by entering the titleS C J P is checked out successfullyMain menu Successful Exit from MenuDo you want to log out?? \n" +
                "Yes(1) No(0)User logged out";
    ...
    public void shouldCallCheckOutBookMenuActionWhen2IsPassedWithoutValidCredentials() throws Exception {
        movieLibraryManager = mock(LibraryManager.class);
        List<Item> list = movieLibraryManager.getItemListByCategory("book");
        subMenu = new SubMenu(testReaderWriter, movieLibraryManager);

        when(movieLibraryManager.displayMainMenu()).thenReturn("Main menu ");
        when(movieLibraryManager.displayItemDetailsInTableForm(list)).thenReturn("Book Details ");
        when(movieLibraryManager.validTitle("S C J P")).thenReturn(true);
        testReaderWriter.consoleInput("2\nlib-1001\nasds\n4\n0");

        String expected = "Main menu Enter library numberEnter passwordNot a valid userMain menu Successful Exit from Menu";

        subMenu.menu(list, movieLibraryManager, testReaderWriter, manager);
    ...
    public void shouldCallReturnBookMenuActionWhen3IsPassedWithValidCredentials() throws Exception {
        ...
        String expected = "main menuEnter library numberEnter passwordbooksSelect a Item by entering the titleS C J P returned to the librarymain menuSuccessful Exit from MenuDo you want to log out?? \n" +
                "Yes(1) No(0)";
        subMenu.menu(list, bookLibraryManager, testReaderWriter, manager);
        assertEquals(expected, testReaderWriter.consoleOutput());
